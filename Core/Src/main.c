/* USER CODE BEGIN Header */ C/**** ******************************************************************************
 * @file              : main.c
 * @brief             : Main program body (Final Corrected Version with All Original Functions Preserved)
 ******************************************************************************
 */
...
//#define CMD_SERVO_TAIL_TEST      12 // 新增：尾部舵机测试指令
...
void Execute_Test_Movement(void)
{
    ...
    base_tail_pulse = current_tail; // 新增
...
void Start_Servo_Range_Test(uint8_t axis_mode, int nod_A, int nod_B, int shake_A, int shake_B, uint32_t duration_ms, uint8_t repetitions)
{
    ...
    if (axis_mode == AXIS_NOD) {
        ...
        g_test_target_shake = base_shake_pulse;
         // 设置尾部舵机的测试范围 (A点和B点)
        // 注意：需要复用传进来的 nod_A 和 nod_B 参数来传递尾部舵机的位置
...

{
  ...
  HAL_TIM_PWM_Start(&htim12, TIM_CHANNEL_2);
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_4);
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_3);   // <--- 在这里增加这一行
